...
trait SpecializedValidations
{
     ...
     * @return bool
     */
    public static function isZipCode(string $data): bool
    {
        return preg_match("/^[0-9]{5}(?:-[0-9]{4})?$/", $data);
    }

    /**
     * Validates Canadian postal code format (insensitive). The characters are arranged in the form ‘ANA NAN’, where ‘A’
     * represents an alphabetic character and ‘N’ represents a numeric character (e.g., K1A 0T6). The postal code uses
     * 18 alphabetic characters and 10 numeric characters. Postal codes do not include the letters D, F, I, O, Q or U,
     * and the first position also does not make use of the letters W or Z.
     *
     * @param string $data
     * @return bool
     */
    public static function isPostalCode(string $data): bool
    {
